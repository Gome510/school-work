...
void set(List L, long x){
   ...
   if((length(L)>0) && (index(L)>=0)){
      ...
      printf("List Error: calling get() on ((length(Q)<=0) || (index(Q)<0))");
      exit(1);
   }
}
...
void insertBefore(List Q, long data){
   Node N = newNode(data);
...
void insertAfter(List Q, long data){
   Node N = newNode(data);
